...
The card lives in the dashboard DB — there is NO YAML file. Every "sync" action below is an `mcp__danx_dashboard__issue_*` tool call; those tools write the DB directly (it IS the store — no mirror, no file event, no save verb to call after). Read current card state with `mcp__danx_dashboard__issue_get({id})`. Lifecycle (status) changes go through `issue_transition`; prose / AC changes through `issue_edit`; comments through `issue_comment`; retro through `issue_retro`. Agents touch card state ONLY through these tools — never via `Edit`/`Write`/`Read` of any file. Match `danxbot:issue-card-workflow` terminology.

After every commit:

1. **Check off completed AC items** — For each `ac[i]` that this commit satisfies, set its `checked: true` and call `mcp__danx_dashboard__issue_edit({id, ac})` with the full updated `ac[]` array. Match by exact `title` text. Phases are child cards, NOT an in-card checklist — to mark a phase done, complete its child card (step 4).

2. **Append a commit comment** — Call `mcp__danx_dashboard__issue_comment({id, action: 'add', text})`. The server stamps author + timestamp. Body:
   ```markdown
   ## Phase N Commit
...
4. **Update parent epic** (phase cards only) — If the card's `parent_id` is non-null (or the title contains `>` for phase-card pattern like `Epic > Phase N`):
   ...
   **a. Read the epic:** `mcp__danx_dashboard__issue_get({id: <parent-id>})`.

   **b. Check off epic AC:** The phase is reflected as done automatically once this session's child card is completed (step 3) — the epic has no per-phase status array to check off; phase-done derives from the child card's own status. Set `ac[i].checked: true` for any epic-level AC items this phase satisfies and persist via `mcp__danx_dashboard__issue_edit({id: <parent-id>, ac})` — epic AC items map to specific phases and must be marked complete as the work is verified, not deferred until the epic is fully done.

   **c. Append a Phase Handoff comment** to the epic via `mcp__danx_dashboard__issue_comment({id: <parent-id>, action: 'add', text})`. This is the bridge between agents — it ensures no knowledge is lost when context is destroyed. Body:
   ```markdown
   ## Phase N Handoff
   ...
   **d. Re-read the epic description and remaining phase cards.** If anything is wrong, outdated, or missing context from what you learned during this phase, update it now via `mcp__danx_dashboard__issue_edit({id, description})` (the epic; or the sibling phase child cards by their ids). The epic must always be zero-context ready for the next agent.

   **e.** (No separate persist step — `issue_edit` / `issue_comment` already wrote the DB.)

   **f. Update the next phase card.** Read each child id from the epic's `children[]` (`mcp__danx_dashboard__issue_get`) until you find the next phase still in `ToDo` / `In Progress` with unchecked work. Then call `mcp__danx_dashboard__issue_comment({id: <next-child-id>, action: 'add', text})` to append a "Notes from Phase N" entry covering anything that could cause the next agent to waste time or make mistakes: discovered constraints, timing gotchas, reusable helpers and their paths, cost/budget observations, dependencies between phases. The next agent has ZERO context — it reads only the card's description and comments.

   **g. Check if epic is complete.** If every phase child card is complete AND every epic `ac[i].checked: true`, fill the epic's retro via `mcp__danx_dashboard__issue_retro` (step 5) and call `mcp__danx_dashboard__issue_transition({id: <parent-id>, action: 'complete', summary})`. Do not leave the epic In Progress or ToDo when all phases are Done.

5. **Filling `retro` on terminal** (Done / Cancelled / Blocked). Call `mcp__danx_dashboard__issue_retro({id, good, bad, action_item_ids, commits})` BEFORE the terminal `issue_transition`. The server auto-renders the `## Retro` comment. Fields:
...
- **NEVER call `mcp__trello__*` tools from agent path** — the danxbot worker is the sole writer to the backend tracker; the agent path is `mcp__danx_dashboard__issue_*` only
