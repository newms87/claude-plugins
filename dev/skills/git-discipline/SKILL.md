---
name: git-discipline
description: 'MANDATORY before ANY `git` command, no exceptions for "read-only" or "low-stakes". Loads safety rules (no destructive ops without approval, no checkout/restore/revert, never delete repos, push after commit, no cherry-pick/apply/worktree-fork without current-HEAD base, pre-push diverged-branch handling without menu-asking the operator) as TodoWrite checklist. SELF-TRIGGER GATE — fires on YOUR OWN draft, not just user phrasing. Before composing any Bash tool call whose command contains the token `git ` (with trailing space) — INCLUDING `git status`, `git diff`, `git log`, `git show`, `git branch`, `git remote`, `git fetch`, `git pull`, `git pull --rebase`, `git push`, `git commit`, `git add`, `git stash`, `git checkout`, `git restore`, `git revert`, `git reset`, `git clean`, `git cherry-pick`, `git apply`, `git rebase`, `git merge`, `git worktree`, `git tag`, `git rm`, `git mv` — STOP and confirm `dev:git-discipline` is loaded THIS turn. "Just a quick status check" / "just reading the log" / "git is harmless before mutating ops" are the exact rationalizations the gate exists to block — read-only ops gate the same as mutating ops because the agent uses their output to decide the next (mutating) action, and the skill body carries the diverged-branch + menu-ask rules that fire off `git status` output. Also fires on: about to delete files; about to dispatch an Agent with isolation:"worktree"; about to integrate work from another worktree or branch.'
---
