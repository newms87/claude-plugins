...
- **Data lives in plain JS objects/arrays** near the top of the `<script>` block — one array per tab, one object per entry (`{id, title, body, status, tags}`, or the richer shape decisions/timeline need).
