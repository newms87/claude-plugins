...
After editing: commit + push from the source checkout (`git add <plugin>/... && git commit && git push`) so other instances + container workers (`autoUpdate: true`) pick up the change. If the repo ships a publish script, use it — a plain push may not bump the version the marketplace loader compares.
