---
name: investigate
description: MANDATORY when investigating, diagnosing, or asserting facts about a running system WITHOUT applying a fix. Triggers — user says "investigate," "look into," "find out," "dig into," "figure out," "trace," "audit," "check on"; user asks "why is X happening", "how does Y work", "what's going on with Z", "is the agent running", "did it dispatch"; user requests a **call-chain trace** (e.g., "trace the path from X to Y"), **code-path walkthrough** (e.g., "walk through how X decides"), **read-only inventory** (e.g., "which workers are running right now"), **read-only comparison** (e.g., "difference between X and Y", "when does each fire"), **read-only behavioral analysis** (e.g., "understand how X falls back"), or **environment-specific behavior mapping** (e.g., "how does X resolve differently in prod vs. local", "how does the worker resolve PORT in production versus dev"); about to STATE a factual claim about timing, latency, runtime behavior, causality, config values, process state, fallback logic, decision criteria, or "why X works this way" with anything more than a direct file quotation. Use INSTEAD OF the debugging skill when the assignment is read-only — diagnosis only, no code changes, no failing test, no fix, no modifications to running state (no restarts, no deployments, no toggles, no edits). If the task crosses into "fix it now" or "modify the running system," switch to the debugging skill (load it via Skill tool) and follow the full TDD discipline. **When reporting investigation findings about a bug / problem / failure to the user, the per-bug Affects/Env/Scenario/Expected/Actual format from the `debugging` skill Phase 12 is REQUIRED — load `debugging` for the format spec.** Loads investigation discipline as TodoWrite checklist.
---
