---
name: investigate
description: 'MANDATORY ONLY when the request is read-only diagnosis with NO accompanying mutation in the same prompt. Decision rule before invoking: scan the WHOLE prompt for any clause asking to fix / patch / apply / add / build / write / create / rewrite / refactor / modify / change / extract / move / install / enable / disable anything — if ANY such clause is present, this skill does NOT fire and the request belongs to the `debugging` skill (TDD discipline). Fires on (read-only-only): user says "investigate," "look into," "find out," "dig into," "figure out," "trace," "audit," "check on"; user asks "why is X happening", "how does Y work", "what''s going on with Z", "is the agent running", "did it dispatch"; about to STATE a factual claim about timing, latency, runtime behavior, causality, config values, process state, or "why X works this way" with anything more than a direct file quotation. Does NOT fire on (route to `debugging`) when an investigate-verb is paired in the SAME prompt with any mutation tail: "and fix it," "and patch the bug," "and apply the fix," "and apply the patch," "and add a/the X," "and add the missing X," "and add Y you find," "and build X," "and write X," "and create X," "and rewrite X," "and refactor X," "and extract X," "and modify X," "and change X," "once you find it," "once you''ve located it," "in the same dispatch," "in this same session," "in the same session," "while you''re in there." When reporting investigation findings about a bug / problem / failure to the user, the per-bug Affects/Env/Scenario/Expected/Actual format from the `debugging` skill Phase 12 is REQUIRED — load `debugging` for the format spec. Loads investigation discipline as TodoWrite checklist.'
---
