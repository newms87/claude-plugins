---
name: investigate
description: "MANDATORY when investigating, diagnosing, or asserting facts about a running system WITHOUT applying a fix. Triggers — user says \"investigate,\" \"look into,\" \"find out,\" \"dig into,\" \"figure out,\" \"trace,\" \"audit,\" \"check on\"; user asks \"why is X happening\", \"how does Y work\", \"what's going on with Z\", \"is the agent running\", \"did it dispatch\"; user requests read-only analysis of call chains, runtime behavior, worker/process inventory, detector/component interactions, configuration fallback logic, environment-specific behavior differences, feature-flag resolution, or port/environment variable resolution across contexts; user wants to understand or walk through how a system component decides or behaves (e.g., \"how does the watcher decide when to mirror\", \"how does the feature flag resolver fall back\"), including tracing message/request paths through listeners and handlers, comparing when different detectors or components fire, and auditing decision logic in watchers, resolvers, or dispatchers; about to STATE a factual claim about timing, latency, runtime behavior, causality, config values, process state, feature flags, worker inventory, system paths, environment-variable fallback chains, or \"why X works this way\" with anything more than a direct file quotation. **Crucially: INCLUDE call-chain tracing (e.g., \"trace the path a message takes from listener to reply\" — follow execution across function boundaries), worker/process read-only inventory checks (e.g., \"which workers are running on target X right now\" — snapshot of live processes, no restarts), comparative behavior analysis (e.g., \"when does detector A fire vs. detector B\" — document trigger conditions), decision-logic walk-throughs (e.g., \"how does the watcher decide\" — step through conditional branches and state checks), and environment-specific resolution (e.g., \"how is this port resolved in prod vs. dev\" — trace config load and fallback chains). Use INSTEAD OF the debugging skill when the assignment is read-only — diagnosis only, no code changes, no failing test, no fix, no modifications to runtime state. If the task crosses into \"fix it now,\" switch to the debugging skill (load it via Skill tool) and follow the full TDD discipline. **When reporting investigation findings about a bug / problem / failure to the user, the per-bug Affects/Env/Scenario/Expected/Actual format from the `debugging` skill Phase 12 is REQUIRED — load `debugging` for the format spec.** Loads investigation discipline as TodoWrite checklist."
---
