---
name: investigate
description: "MANDATORY when investigating, diagnosing, or asserting facts about a running system WITHOUT applying a fix. Triggers — user says \"investigate,\" \"look into,\" \"find out,\" \"dig into,\" \"figure out,\" \"trace,\" \"audit,\" \"check on\"; user asks \"why is X happening\", \"how does Y work\", \"what's going on with Z\", \"is the agent running\", \"did it dispatch\"; user requests **call-chain trace** (e.g., \"trace the path from listener to handler — call-chain only\", \"trace a Slack message from bolt to danxbot_slack_reply — call-chain only, no edits\"), **code-path walkthrough** (e.g., \"walk through how X decides\", \"audit how the chokidar watcher in src/db/issues-mirror.ts decides when to mirror\"), **read-only inventory** (e.g., \"which workers are running right now\", \"which workers are currently running on the gpt target right now\"), **read-only comparison** (e.g., \"difference between X and Y\", \"when does each fire\", \"dig into the difference between StallDetector and ApiErrorDetector — read-only comparison of when each fires\"), **read-only behavioral analysis** (e.g., \"understand how X falls back\", \"how does isFeatureEnabled fall back when settings.json is malformed — I want to understand it, not modify it\"), or **environment-specific behavior mapping** (e.g., \"how does X resolve differently in prod vs. local\", \"how does the worker resolve DANXBOT_WORKER_PORT differently in production versus dev\"); also MANDATORY when about to STATE a factual claim about timing, latency, runtime behavior, causality, config values, process state, fallback logic, decision criteria, or \"why X works this way\" with anything more than a direct file quotation. The discriminator: is the assignment read-only (diagnosis, tracing, understanding, inspection only) with **no code changes, no test fixes, no running-state modifications** (no restarts, no deployments, no toggles, no edits, no \"fix it now\")? If YES → investigate. If the task crosses into \"fix it,\" \"change it,\" \"make it work,\" or \"modify the running system,\" switch to the debugging skill (load it via Skill tool) and follow full TDD discipline. **When reporting investigation findings about a bug / problem / failure to the user, the per-bug Affects/Env/Scenario/Expected/Actual format from the `debugging` skill Phase 12 is REQUIRED — load `debugging` for the format spec.** Loads investigation discipline as TodoWrite checklist."
---
