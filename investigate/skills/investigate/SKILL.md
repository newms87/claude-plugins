...
name: investigate
description: |-
  MANDATORY when investigating, diagnosing, or asserting facts about a running system WITHOUT applying a fix. Triggers — user says "investigate," "look into," "find out," "dig into," "figure out," "trace," "audit," "check on"; user asks "why is X happening", "how does Y work", "what's going on with Z", "is the agent running", "did it dispatch"; user requests read-only analysis of call chains, runtime behavior, worker/process inventory, detector/component interactions, configuration fallback logic, environment-specific behavior differences, feature-flag resolution, or port/environment variable resolution across contexts.

  **CRUCIALLY INCLUDE all of the following when present in the request:**

  1. **Call-chain tracing** — user wants to "trace the path" a message/request/event takes from entry point (listener, webhook, handler) through intermediate functions to exit point (reply, dispatch, emit) — follow execution across all function boundaries without modifying code.
  2. **Worker/process read-only inventory** — user asks "which workers are running on [target] right now", "what processes are live", "snapshot of [component] state" — document active processes/workers WITHOUT triggering restarts or state changes.
  3. **Comparative behavior analysis** — user asks "when does [detector/component A] fire vs. [detector/component B]", "difference between X and Y", "when each triggers" — document and compare trigger conditions and firing patterns side-by-side WITHOUT code edits.
  4. **Decision-logic walk-through** — user asks "how does [watcher/resolver/dispatcher] decide", "walk through the logic", "step through the conditions" — trace conditional branches, state checks, and decision points WITHOUT modifying behavior.
  5. **Environment-specific resolution** — user asks "how is [config/port/env-var] resolved in prod vs. dev", "fallback chain in production", "different behavior across contexts" — trace config load order, environment variable substitution, and fallback chains WITHOUT applying fixes.
  6. **Feature-flag/config understanding** — user asks "how does isFeatureEnabled fall back", "what happens when config is malformed", "how is this setting resolved" — document resolution and fallback logic WITHOUT modifying the system.

  Use INSTEAD OF the debugging skill when the assignment is read-only — diagnosis only, no code changes, no failing test, no fix, no modifications to runtime state. If the task crosses into "fix it now," switch to the debugging skill (load it via Skill tool) and follow the full TDD discipline. **When reporting investigation findings about a bug/problem/failure to the user, the per-bug Affects/Env/Scenario/Expected/Actual format from the `debugging` skill Phase 12 is REQUIRED — load `debugging` for the format spec.** Loads investigation discipline as TodoWrite checklist.
---
