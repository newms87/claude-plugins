...
// desktop app's in-app browser and reopen it whenever it goes missing — but
// that was only an instruction, nothing enforced it. This hook enforces the
// "reopen it" half mechanically.
//
// WHY NOT A DIRECT MCP CALL. An `mcp_tool` hook could in principle call
// `mcp__Claude_Browser__tabs_context` itself with no model turn needed — but
// `mcp_tool` hooks cannot run at all on SessionStart/Setup, and whether a
// host-provided server (the desktop app's own Browser pane, never declared
// in any `.mcp.json`) is addressable as "a configured MCP server" the way a
// declared one is remains unverified. So this hook does not gamble on it: it
// puts the check in the model's hands via `additionalContext` instead of an
// unverified autonomous MCP call.
//
...
//   session). Then, unless nudged within SUPPRESS_MS, emits ONE
//   additionalContext directive telling the model to check tabs and reopen if
//   needed, and stamps the nudge time. The stamp is OPTIMISTIC (written on
//   asking, not on a confirmed reopen) because a UserPromptSubmit hook cannot
//   observe what the model does after its turn starts. This bounds "at most
//   one check/reopen nudge per turn" trivially (one hook invocation per
//   prompt) and keeps the no-plan and suppressed cases byte-cheap (empty
//   stdout, no model context added).
