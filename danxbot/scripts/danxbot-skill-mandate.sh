...
(2) danxbot:issue-blocker — about to stamp `blocked: {at, reason}`, populate `waiting_on[]`/`conflict_on[]`, recommend Blocked, escalate to a human via `issue_problem` add (a card needs a human exactly when `open_problem_count > 0`), call danxbot_complete with "operator must X" framing, assess whether a blocker is genuine vs ambiguous/recoverable, or pick up a Needs Help / Blocked card ("unblock"/"get unstuck"/"what does this need").
(3) danxbot:autonomous-mode — dispatched to worker (DANXBOT_REPO_NAME set); no AskUserQuestion, no plan-mode pause, one exit via danxbot_complete.
(4) danxbot:halt-flag — a board_halts row is raised, dispatch halted, or signaling `danxbot_complete({status:"critical_failure"})`.
(5) danxbot:danxbot — touching <repo>/.danxbot/, running make launch-worker/deploy, investigating stuck dispatch, explaining dispatch runtime. About to run `make launch-worker`/`make launch-all-workers`/`make deploy*`/any worker or prod start, or a "production unreachable" claim: that repo's own `.claude/rules` (R-17) is now the launch/prod-access authority, not a plugin skill — read it before acting.
(6) danxbot:db-reset — destructive DB reset (`migrate:fresh`, `DROP DATABASE`, etc.).
