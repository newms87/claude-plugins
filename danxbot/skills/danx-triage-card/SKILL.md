...
4. **Act:**
   ...
   - **Blocked / Waiting On** → no `issue_triage` call (there is no confidence question for a blocker or dependency-wait audit — see references/triage-paths.md for the exact calls, including the escalation a Confirmed human-only block needs).
...
6. **Complete** — `danxbot_complete({status: "complete", summary})` ALWAYS, regardless of outcome. The card's fate is already fully driven by step 4 (`issue_triage` for Review, the noted transition calls for Blocked/Waiting On). `danxbot_complete`'s status is ONLY "did this dispatch itself succeed" — a triage agent that read the card and recorded a real, evidenced decision succeeded, no matter which outcome it reached. Never call it with `status: "ready"` / `"cancelled"` / `"archive"` — that stamps the DISPATCH row `failed` even though the triage succeeded (only literal `"complete"` counts as dispatch success). Use `status: "failed"` (with a real ≥30-char reason) ONLY when the dispatch itself couldn't complete — e.g. the card was unreadable, an MCP call errored, you genuinely could not reach a decision.

## Confidence Rubric (Review only)

**One integer, 0–5. RELEVANCE/UTILITY ONLY — never effort, ease, or implementation cost.** The server, not you, turns this into Cancel/Park/Keep/Approve by comparing it to the board's configured thresholds (defaults: `0` → Cancel, `1–2` → Park, `3–4` → Keep, `5` → Approve — thresholds are board-configurable, DX-2080, so treat these as illustrative, not fixed). Park and Keep both escalate to the operator via an open problem (DX-2782) rather than blocking the card — opening the problem is the entire escalation; there is no separate flag to set.
...
- For Blocked: audit does not do the escalation's steps — if misclassification found, Demote (via `issue_transition`) + let next dispatch DO the steps. `blocked` is a hold that never reaches the operator: when a Confirmed card has a genuinely human-only step but no open problem yet, escalate it (`issue_problem add`) so it lands in Needs You.
