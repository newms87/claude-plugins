...
description: 'Issue card lifecycle: status derivation, mcp__danx-dashboard__issue_* tools, comment/retro/blocked/waiting_on contracts, container-atomic rule (Epic + Feature). LOAD BEFORE proposing card TYPE or slicing work into cards (epic/feature/story) — recommending a structure in chat is the gated action; naming a type before loading this skill decides it wrong.'
---
...
This file writes the **hyphen** form throughout because the dispatched-worker path is the majority consumer. **In an operator session those exact names do not exist** and calling them returns "No such tool available."
...
| Blocked (dispatch paused, resolvable in-repo) | `issue_transition({id, action:'block', reason})` | `danxbot_complete({status:'failed', summary})` |
| Needs a human to decide, or to act (question vs action — see below) | `issue_problem({id, action:'add', statement, type?, summary?, solutions?})` — zero solutions is valid | `danxbot_complete({status:'complete', summary})` |

Skipping Step A leaves the card stuck mid-state — `dispatch_id` cleared (worker side), but no lifecycle stamp. Verify via `issue_get` after Step A: `completed_at` / `cancelled_at` / `blocked_at` non-null (or, for an escalation, `open_problem_count > 0` via `issue_get({id, fields:['problems']})`) + `status_derived` matches expected, BEFORE Step B. There is no `issue_requires_human` tool and no separate "set" step — opening the problem IS the whole escalation (DX-2830).
...
`summary` descriptions (MCP Tools Reference below).
...
**SMALL EDITS ARE THE EXCEPTION — do them, never card them.** In an interactive session with the operator, a small, clear edit is made right away: no card, no `ready`, no dependency wiring. A card for a one-line CSS change is ceremony that costs the operator a round-trip and buys nothing. Mechanical test: *can you name the file(s) and the exact lines, is it a few lines, and is there no design decision left to make?* All yes → edit it now (another session working in the same file doesn't change that; edit only your lines). Any no → the gate above applies, and a card is the right vessel.

What the gate still blocks is **substantial** work taken inline because it feels convenient: "exploratory", "lower-latency", "I'm already here" on anything beyond a small edit. "Help me fix this" / "this is broken" for a non-trivial fix is a card request, not an inline instruction.
...
them itself, inline or via its own sub-agents, as the PRIMARY path — see its "Actionable work"
section — and readies each one anyway so an idle worker can still claim it opportunistically.
This "DEFAULT IS ALWAYS" gate stays exactly as written above for every other context: no
connected plan, an ad hoc operator request, or any card outside a plan's own scope. When unsure
which the operator wants, ASK — never assume build-in-session. Self-pickup (below) applies ONLY
once in-session build is confirmed (explicitly, or by the Plan exception).
...
1. `issue_transition({id, action: 'pickup', manual: true, assigned_agent: '<your identity>'})` → `In Progress` (`dispatchable_derived: false`). This REMOVES the card from the poller's dispatch pool. **`assigned_agent` IS the claim — a pickup without it does not take the card.**
   **MECHANICAL CHECK, before your next tool call of any kind: the pickup RESPONSE already carries the card row — read `assigned_agent` OUT OF THAT RESPONSE.** Anything other than you — **`null` included** — means **the claim did NOT land and the card is now In Progress and OWNERLESS.** Re-issue the pickup; if it comes back unowned again, STOP and tell the operator. Passing `assigned_agent` is a REQUEST; the value echoed in the response is the only evidence it was recorded. A 200 does not imply the identity persisted.
...
4. `manual: true` bypasses every card-flow gate (`ready_at`, `blocked_at`, `depends_on`, `conflict_on`) EXCEPT `open_problem_count` — a manual pickup is fully operator-controlled and works straight from Review, but a card with any open problem (needs a human) never starts, manual or not. Only terminal / already-dispatched still refuse 409 otherwise.
...
2. **Quality gates — record a VERDICT with `issue_quality_gate_verdict`.** Run your own equivalent review (architecture-reviewer / code-reviewer / test-reviewer subagents, or inline review for a small change) FIRST. Then, for each POST gate the card carries (`code-architecture` / `code-quality` / `code-test-quality`), call:
   ...
   **The two gate tools are SIBLINGS and neither substitutes for the other:**

   | Tool | Writes | Effect on `complete` |
   |---|---|---|
   | `issue_quality_gate({id, gate, action: 'add'\|'remove', note?, effort_level?})` | whether the gate is **on the card at all** — `add` puts it there (running), `remove` discards the row AND any verdict on it | Adding/removing does not itself pass anything; it only decides which gates `complete` checks. |
   | `issue_quality_gate_verdict({id, gate, status, message})` | the **VERDICT** (did it pass) | **This is what unblocks `complete`.** |

   Every gate on a card is required by definition — there is no separate "required flag" to flip. `issue_transition complete` 409s (`failed_gate: "quality_gate_post"`, `failed_post_gates[]`) while any POST gate on the card is not `pass`. If a POST gate genuinely does not apply, `issue_quality_gate({action:'remove'})` it rather than trying to satisfy it — do not invent a passing verdict for a gate that shouldn't exist on this card.
...
**Dashboard DB** (via `mcp__danx-dashboard__issue_*` MCP tools) is the canonical source for title, description, status, AC, children, comments, retro, blocked, waiting_on, and problems. There is no settable "requires human" field — a card needs a human exactly when `open_problem_count > 0`, computed automatically from the `problems` field group; agents escalate by opening a problem, never by writing a flag. Agents read + write via MCP only — that is the whole surface. Poller dispatches off the dashboard DB via the dashboard HTTP API. Want a status change → call `mcp__danx-dashboard__issue_transition` or `mcp__danx-dashboard__issue_edit`.
...
- **Durable work-records live on a card — never a substitute artifact.** (WHEN this fires lives in the SKILL LOAD MANDATE; this is the contract once loaded.) Any card/epic/plan/findings/design/handoff/spec meant to survive the session = CREATE/APPEND on a tracker card (MCP `issue_create`, or `POST /api/issues`). Substitutes that do NOT count and are forbidden: a standalone repo `.md`, and in-session `TaskCreate`/`TaskList` (ephemeral working memory — mirror steps there only AFTER the card exists). Forbidden target dirs — `docs/handoffs/`, `docs/specs/`, or any dir git history shows was deleted: recreating them is the failure. Create-tooling unavailable → STOP and ask; never default to a parallel `.md`. Standalone docs only when the user explicitly asks for a file/doc. **Composes with `danxbot:plan-workflow`:** in human-driven sessions the connected danxbot Plan is the planning record — its goals/rules/caveats and architecture document hold the plan-level context, and every card created per this contract is attached to it with `plan_add_card`. The card stays the work-record; the plan never duplicates card content.
