...
**Split-signal check on a Story's own description (MANDATORY, run whenever you read or write a Story body):** a Story whose own description already contains **2+ top-level `# Layer` / `# Part <n>` headings**, OR an **`# Appendix`-labeled section whose size rivals its requirements body**, is showing the slice count in its own structure — those headings/sections ARE the slices the count gate above should have separated. Flag it: name the exact headings found and their approximate size (character count or fraction of the body), then re-file it as a **Feature** with each Layer/Part becoming its own child Story (`parent_id`, per the Allowed parent→child type matrix below). Do not silently reshape it — surface the flag with its evidence before proceeding.
