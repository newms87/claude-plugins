...
Each line MUST name a concrete user-observable result; a line with no see/do answer is a horizontal layer → not a Story → fold it. No Slice Plan printed in the turn = DO NOT create the Feature/Epic (and do not create its child cards). The Feature/Epic card is created only AFTER the plan, and its `phase_children` / child cards ARE those Stories. Skipping the plan because "the skill is already loaded" / "I know the slices" / "I'll just type Feature" is the exact compliance failure this forcing function exists to make visible — the plan must appear in the transcript, not in your head.

**DEFAULT, DON'T ASK — pre-`AskUserQuestion` gate when the operator hands a detailed feature request.** A detailed request is a mandate to FILE, not a prompt to interview. Bake every design decision as a STATED default in the cards + your message and file the epic + phases (epic-atomic) — the operator corrects in-thread. Open `AskUserQuestion` for a decision ONLY when it passes this mechanical test: **answering it differently would force the cards to violate a CLAUDE.md Core Principle, and no default resolves that collision.** Ask ONLY the question(s) that pass — never bundle defaultable choices alongside a genuine one (every defaultable rider you attach is the over-ask this gate blocks). "It's a fork, so I'll ask" / "the advisor listed three things" / "cheaper to confirm than redo" are the rationalizations this gate blocks — a fork with a conventional default gets defaulted, stated, and shipped, not asked.
