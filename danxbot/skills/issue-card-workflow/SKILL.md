...
| Needs a human (decision or external action) | `issue_problem({id, action:'add', statement, solutions?})` — zero solutions is valid | `danxbot_complete({status:'complete', summary})` |
...
| `mcp__danx-dashboard__issue_problem({id, action: 'list'\|'add'\|'edit'\|'remove', problem_id?, base_hash?, statement?, solutions?})` | A card's problems, via `/api/issues/:id/problems[/:pid]`. **This is the ONLY way to escalate a card to a human — there is no separate flag or tool.** `list` → live problems in order, each `{id, statement, content_hash, open, solutions[], decisions[]}`. `add` → `{statement, solutions?}` — zero solutions is valid, the operator then answers free-form; returns `problem_id` + `solution_ids` in one transaction, plus `problems_reminder: {open_problem_count, instruction}`. `edit` → `{problem_id, base_hash, statement}`. `remove` → `{problem_id, base_hash}` — ALWAYS allowed, even as the card's last open problem (removing it is simply how a card stops needing a human, DX-2830). Stale `base_hash` → `409 stale_problem` with `currentHash`/`currentProblem` — merge and retry. There is no answer action on any tool — only the operator answers, in the dashboard. |
