...
- **NEVER write cards/epics/plans/findings/designs/hand-offs as a standalone `.md` doc as a substitute for filing them.** This covers ANY durable work record — "write up the cards", "make cards", investigation findings, a bug list, an e2e/test design, a remediation plan, a "handoff". All of it = CREATE/APPEND on a tracker card (MCP, or `POST /api/issues` dashboard API). The trigger is "I want this to persist", NOT only the literal word "card". Pre-write mechanical check before any `Write`/`Edit` to a repo `.md`: (a) Is this a finding/plan/design/handoff meant to survive the session? → tracker card, not a file. (b) Is the target path under `docs/handoffs/`, `docs/specs/`, or any dir git history shows was deleted? → STOP, that dir was removed on purpose; recreating it is the failure. If create-tooling is unavailable, STOP and ask — do NOT default to a parallel `.md`. Standalone docs only when the user explicitly asks for a file/doc.
- **In-session `TaskList`/`TaskCreate` is NOT a substitute for a card — same failure as a standalone `.md`.** Mechanical pre-action gate, fires WITHOUT the word "card": before the FIRST mutating step of any operator-initiated work that is multi-phase, spans sessions, or is a deploy/launch/migration (i.e. anything you'd reach for `TaskCreate` to track), CREATE the tracker card FIRST, then mirror steps into TaskList if useful. TaskList is ephemeral working memory; the card is the durable record. "User didn't say card" / "it's just an interactive session" is the exact rationalization this gate blocks — scope, not keyword, triggers the card.
