---
name: process-kill
description: "MANDATORY before sending any kill/terminate signal to any process — `kill`, `kill -9`, `pkill`, `pkill -f`, `taskkill`, `taskkill /F`, `SIGTERM`, `SIGKILL`, or any signal variant to any PID, process name, or pattern. Workspace shared with user + other agents → wrong kill destroys hours of work irreversibly. Invoke BEFORE running any kill/terminate command, BEFORE \"cleanup\" of stale processes, BEFORE killing anything inferred from `ps`/`pgrep`/`docker top` output, BEFORE terminating processes matched by pattern flags like `-f`. Triggers on ANY explicit or inferred kill intent including: direct subprocess termination (\"kill the claude subprocess\", \"Use `kill -9` on the runaway subprocess\", \"kill the runaway claude subprocess\"), pattern-based termination (\"pkill vitest\", \"Send SIGTERM to every process matching 'vitest'\", \"`pkill -f 'node.*tsx watch'`\", \"Run `pkill -f 'node.*tsx watch'` to clear stale tsx watchers\", \"`pkill vitest`\"), zombie/stale process cleanup (\"clean up stale tsx watchers via pkill\", \"Clean up the stale tsx watchers via pkill — they're zombie processes\"), Windows process termination (\"taskkill /F /PID 5678\", \"`taskkill /F /PID 5678` to kill the Windows Terminal process\", \"Run `taskkill /F /PID 5678`\"), and signal dispatch to specific PIDs (\"Send SIGKILL to PID 12345\", \"Send SIGTERM to PID 12345\", \"Send SIGKILL to PID\", \"Send SIGTERM to every process matching\"). Triggers on backtick-wrapped commands containing kill/pkill/taskkill/SIGKILL/SIGTERM even when framed as narrative description. No \"obviously mine\" exemption, no \"just cleanup\" exemption, no \"no one else could have that\" exemption, no \"it's a zombie\" exemption, no \"runaway\" exemption, no \"stale\" exemption. Loads Iron Rule + Proof Block requirement as TodoWrite checklist."
---
