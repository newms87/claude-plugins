...
A person may have a browser tab open against it right now, and every in-flight request
dies the instant you stop or relaunch it — whether you kill it directly or a helper tool
restarts it for you (a "start/ensure this is running" call that finds one already up and
cycles it has the same effect as a manual kill+relaunch, even with no `kill` command
anywhere in your own commands).

The Iron Rule above answers "may I signal this PID" — ownership and provenance. It does
not answer "is anyone using it right now," which is the more common way this goes wrong.
...
1. Check for a live connection first (an established session/socket on its port, a
   recent request in its access log) rather than assuming the coast is clear.
