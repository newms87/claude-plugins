# Shared UserPromptSubmit machine-authored-turn detection for the `base`
# plugin's always-on mandates (operating-contract.sh, craft-mandate.sh) —
# both need to suppress their per-turn pointer on a turn that did not
# originate with the operator. Extracted here because BOTH consumers live in
# this one plugin install. This file is NOT reusable by any other plugin's
# hooks (human-collaboration's human-loop-mandate.sh, investigate's
