...
| `plugin-skill-mandate.sh` hook wiring | **RESOLVED by deletion (DX-3347, 2026-09-26)** | Script header said "Fires on SessionStart and UserPromptSubmit" and had UserPromptSubmit stdin-drain code, but base/hooks.json wired it SessionStart-ONLY — dead code, not a wiring fix. DX-3347 deleted the file outright, folding its skill-load pointer into `danxbot/mantra.md`'s one-line skill routing (PLN-11 R-16: fold into the mantra, or delete if the skill's own description already carries the trigger). CP-20 should be triaged → Cancel, not implemented as originally scoped (there is no longer a script to re-wire). Original claim preserved for history: ICE 280 (I5×C7×E8). |
...
| MCP tool namespace `mcp__danx_dashboard__` (underscore) | **REVERSED 2026-08-15 — this row was WRONG, CP-29 was RIGHT** | Original claim below (kept for history) asserted underscore is correct and Claude Code sanitizes hyphens to underscores. That is false. Proven three independent ways in a later session: (1) real production JSONL transcripts of dispatched-worker sessions — `mcp__danx-dashboard__*` (hyphen) calls succeeded with real 200 responses, `mcp__danx_dashboard__*` (underscore) calls failed with `"No such tool available"`; (2) the installed Claude Code CLI binary's own tool-name sanitizer, `e.replace(/[^a-zA-Z0-9_-]/g,"_")` — hyphen is in the allowed/negated character class, never replaced, plus a live example in the same binary, `mcp__claude-in-chrome__tabs_context_mcp`, preserving a real product's hyphenated server name verbatim; (3) **official Claude Code docs**, [Connect to external tools with MCP](https://code.claude.com/docs/en/agent-sdk/mcp) — every code example with a hyphenated server name preserves the hyphen in the tool prefix: `allowedTools: ["mcp__claude-code-docs__*"]`, `mcp__remote-api__*`, `mcp__api-server__*`, `mcp__secure-api__*`, `mcp__oauth-api__*`. Root cause of the ORIGINAL wrong row: `docker-deep/SKILL.md` L56 itself had the same underscore typo (now fixed), and the operator's own personal `~/.claude.json` local-scope MCP entry for this repo is independently registered as `danx_dashboard` (underscore) — a SEPARATE config surface from the DB catalog's `artifactId: "danx-dashboard"` (hyphen) that dispatched workers actually materialize from — so underscore genuinely worked in an interactive operator session while being wrong for every dispatched worker, which is what made the original investigation's "verified this session" claim feel true without being true for the surface that mattered. Fixed repo-wide (40 files in danxbot's own repo + 20 files across 3 plugins here) in the same session, deployed to production, confirmed live. CP-29 should NOT be cancelled. Original claim, preserved for history: `danxbot/skills/docker-deep/SKILL.md` L56 authoritatively states the live agent-facing server `@thehammer/danx-dashboard-mcp` has "tools prefixed `mcp__danx_dashboard__`" (underscore), used uniformly across 16+ production skills that demonstrably work. **CP-29's premise (live namespace is the hyphen form `mcp__danx-dashboard__`) is FALSE** — Claude Code sanitizes hyphens in an MCP server name to underscores in the tool prefix. CP-29 should be triaged → Cancel (verified this session via the plugins' own docs + the danx-dashboard-mcp entrypoint). |
...
- **[TRIAGE — CANCEL CP-29] REVERSED 2026-08-15 — DO NOT CANCEL CP-29.** The claim below was wrong; see the corrected CP-49 row above for the full proof (real production JSONL, the CLI's own tool-name sanitizer regex, and official docs at https://code.claude.com/docs/en/agent-sdk/mcp — hyphens in MCP server names are preserved, never sanitized to underscores). CP-29's original premise (hyphen is the real live namespace) was CORRECT. Original (wrong) note, preserved for history: the "hyphen namespace" bug is a false positive; underscore prefix is correct per docker-deep L56. Not re-carded (would duplicate/contradict). Flag to operator for Cancel.
